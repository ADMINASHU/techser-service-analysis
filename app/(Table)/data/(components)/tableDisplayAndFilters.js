...
const TableDisplayAndFilters = ({ processedData, filters, handleFilterChange }) => {
  ...
  const formatData = (row) => {
    ...
    return selectedColumns
      .map((colIndex) => <td key={colIndex}>{row[colIndex] !== undefined ? row[colIndex] : ""}</td>)
      .concat(
  ...
  const branchOptions = filters.region
    ? Array.from(
        new Set(
          processedData
            .slice(1)
            .filter((row) => row[row.length - 4] === filters.region)
            .map((row) => row[row.length - 3])
        )
      )
    : Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 3])));
  return (
    <div>
      <div className="filters">
        <select name="year" value={filters.year} onChange={handleFilterChange}>
          <option value="">Select Year</option>
          {Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 1]))).map(
            (year) => (
        ...
        <select name="month" value={filters.month} onChange={handleFilterChange}>
          ...
          {Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 2]))).map(
            (month) => (
        ...
        <select name="region" value={filters.region} onChange={handleFilterChange}>
          ...
          {Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 4]))).map(
            (region) => (
        ...
        >
          ...
          {Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 8]))).map(
            (nature) => (
        ...
        <select name="newStatus" value={filters.newStatus} onChange={handleFilterChange}>
          ...
          {Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 6]))).map(
            (status) => (
        ...
        <select name="assignedTo" value={filters.assignedTo} onChange={handleFilterChange}>
          ...
          {Array.from(new Set(processedData.slice(1).map((row) => row[row.length - 5]))).map(
            (assigned) => (
      ...
      <table border="1" cellPadding="5">
        ...
        <tbody>
          {filteredData.slice(1).map((row, rowIndex) => (
            <tr key={rowIndex}>{formatData(row)}</tr>
          ))}
        </tbody>
      </table>
    </div>
